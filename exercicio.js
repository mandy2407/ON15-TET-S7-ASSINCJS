...
async function resultadoParcelas(produto) {
  try {
    const valor = await buscarPreco(produto)
    const parcela = await calcularParcela(valor.preco)
    return console.log(`Seu ${valor.nome} custa ${valor.preco} e você pode pagar até 10 parcelas de ${parcela} sem juros!"`)
  }
  catch (error) {
    console.log(error)
  }
}
resultadoParcelas("lace")
...
async function calcularValorEmReal(precoEmDolar) {
  try {
    ...
    const valorReal = await buscarJurosImportacao()
    const resultado = await precoDolar.comercial * precoEmDolar
    const resultadoFinal = resultado + (resultado * valorReal.juros1) + (resultado * valorReal.juros2)
    return console.log(`O preço final do produto é ${resultadoFinal.toFixed(2).replace(".", ",")}`)
  }
